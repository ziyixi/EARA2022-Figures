...
def vol_plot_base(conf: dict) -> None:
    ...
    def generate_offset() -> dict[str, np.ndarray]:
        ...
        for row in range(2):
            for col in range(2):
                ...
                res['yabs'][row][col] = f"f{(1-row)*conf['y_offset']+conf['yabs_offset']+8:.3f}i"
                res['ytopo'][row][col] = f"f{(1-row)*conf['y_offset']+conf['ytopo_offset']+8:.3f}i"
        return res
    ...
    def plot_topo(fig: pygmt.Figure, offset: dict[str, np.ndarray], row: int, col: int, info: dict, annote: str, grd_topo: xr.DataArray) -> None:
        ...
        with pygmt.config(MAP_FRAME_TYPE="plain", MAP_TICK_LENGTH="0p"):
            fig.basemap(projection=f"X{conf['x_fig']}i/1i",
                        region=f"0/{conf['length']}/-5000/4000", frame=["lsrn", 'ya2500f', f'pxc{annote}'])
    ...
    def plot_text(fig: pygmt.Figure, idx: int) -> None:
        if idx == 0:
            ...
            fig.text(x=9, y=-1500, text=f"Huabei Plain",
            ...
            fig.text(x=20, y=-1500, text=f"Japan Sea",
                     font="16p,Helvetica,red")
        ...
        if idx == 1:
            ...
            fig.text(x=14, y=1500, text=f"Sichuan Basin",
                     font="16p,Helvetica,red")
            fig.text(x=22, y=1500, text=f"Huabei Plain",
                     font="16p,Helvetica,red")
            fig.plot(x=5, y=2500, style="kvolcano/0.5", color="red")
        if idx == 2:
            fig.text(x=7, y=-1500, text=f"Changbai Mountains",
                     font="16p,Helvetica,red")
            fig.text(x=13, y=-1500, text=f"Japan Sea",
                     font="16p,Helvetica,red")
        ...
        if idx == 3:
            ...
            fig.text(x=20, y=-1500, text=f"South China Sea",
                     font="16p,Helvetica,red")
    ...
    def plot_vectors(fig: pygmt.Figure, idx: int) -> None:
        ...
        if idx == 0:
            fig.plot(x=[10, 2, 2], y=[200, 200, 650], style=style, pen="0.04i,magenta", direction=[
                     [7, 4, 4], [350, 200, 650]])
            fig.text(x=[8.5, 2.5, 2.5], y=[275, 200, 650], text=[
                     "1", "2", "3"], fill="white", font="14p,Helvetica-Bold,black")
        if idx == 1:
            fig.plot(x=[3, 5, 8], y=[500, 400, 400], style=style, pen="0.04i,magenta", direction=[
                     [2, 5, 9], [200, 200, 200]])
            fig.text(x=[2.5, 5, 8.5], y=[350, 300, 300], text=[
                     "1", "2", "3"], fill="white", font="14p,Helvetica-Bold,black")
        if idx == 2:
            fig.plot(x=[4, 3, 3, 10], y=[200, 700, 800, 800], style=style, pen="0.04i,magenta", direction=[
                     [6, 5, 5, 8], [200, 550, 800, 660]])
            fig.text(x=[5, 4, 4, 9], y=[200, 625, 800, 730], text=["1", "2", "3", "4"],
                     fill="white", font="14p,Helvetica-Bold,black")
        if idx == 3:
            fig.plot(x=[12], y=[800], style=style, pen="0.04i,magenta",
                     direction=[[14], [600]])
            fig.text(x=[13], y=[700], text=["1"], fill="white",
