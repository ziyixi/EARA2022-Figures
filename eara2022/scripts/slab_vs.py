...
def generate_offset() -> dict[str, np.ndarray]:
    ...
    for row in range(3):
        for col in range(3):
            res['x'][row][col] = f"f{col*conf['x_offset']+0.8}i"
            res['y'][row][col] = f"f{(2-row)*conf['y_offset']+2.5}i"
            res['yabs'][row][col] = f"f{(2-row)*conf['y_offset']+conf['yabs_offset']+2.5}i"
            res['ytopo'][row][col] = f"f{(2-row)*conf['y_offset']+conf['ytopo_offset']+2.5}i"
    return res
