...
def prepare_data(
    psf_list_path: str, psf_nc_path: str, mask_path: str
    ...
    for category in categories_mapper:
        ...
        for dep in depths:
            res[category][dep] = (
                categories_mapper[category]
                .interp(depth=dep, latitude=hlat, longitude=hlon)
                .T
            )
    return res
...
def main() -> None:
    ...
    psf_nc_path = resource(["model_files", "psf_vsv_bulk_iter19.nc"], normal_path=True)
            ...
            for idep, dep in enumerate(depths):
                with pygmt.config(MAP_FRAME_TYPE="plain", MAP_TICK_LENGTH="0p"):
                    fig.basemap(
                        region=[82, 155, 10, 58],
                        projection="M?",
                        panel=[idep, icategory],
                    )
                pygmt.makecpt(
                    cmap=resource(["cpt", "dvs_6p_nan.cpt"]),
                    series=series[icategory],
                    continuous=True,
                    background="o",
                )
                ...
                fig.plot(
                    data=resource(["Plate_Boundaries", "nuvel1_boundaries"]),
                    pen="2p,red",
                )
                fig.plot(
                    data=resource(["China_blocks", "block2d_mod.txt"]), pen="1p,green4"
                )
                fig.plot(
                    data=resource(["China_blocks", "China_Basins"]), pen="1p,green4"
                )
                fig.text(
                    position="TR",
                    text=f"{dep} km",
                ...
                fig.text(
                    position="BR",
                    text=f"V@-{texts[icategory]}@-",
                    font="28p,32,black",
                    offset="j0.1i/0.15i",
                )

                with pygmt.config(
                    MAP_FRAME_TYPE="inside", MAP_TICK_LENGTH_PRIMARY="10p"
                ):
                    fig.basemap(
                        region=[82, 155, 10, 58],
                        projection="M?",
